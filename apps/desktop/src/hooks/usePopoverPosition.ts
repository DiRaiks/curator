...
/** Vertical side of the anchor the popover should sit on. */
 ...
 * window, scrolls a parent container, or the anchor itself shifts (the
 * bottom-of-screen RunPanel resizing is the canonical case).
 *
...
export function usePopoverPosition(
  ...
  useLayoutEffect(() => {
    ...
    const compute = () => {
      ...
      // Vertical anchoring. `placement: "top"` means the popover sits
      // ABOVE the anchor, so we pin its bottom edge to the anchor's
      // top edge (minus offset).
      if (placement === "top") {
